...
export async function GET(context: any) {
  ...
  const movieItems = movies.slice(0, 25).map((m: any) => {
    ...
    const castLinks   = castNames
      .map((name: string) =>
        `<a href="${escapeXml(`${currentSite}/movie/cast/${encodeURIComponent(name.trim())}`)}">${escapeXml(name)}</a>`
      )
      .join('، ');
    const directorHtml = m.data.director
      ? `<p><strong>إخراج:</strong> ${escapeXml(m.data.director)}</p>`
      : '';
    const castHtml = castLinks
      ? `<p><strong>طاقم العمل:</strong> ${castLinks}</p>`
      : '';
    const overview = escapeXml(m.data.overview || `شاهد فيلم ${m.data.title} أون لاين على معاك سيما`);
    ...
    return {
      ...
      content:     `<img src="${escapeXml(thumbUrl)}" alt="${escapeXml(m.data.title)}" /><br/>${directorHtml}${castHtml}<p>${overview}</p>`,
  ...
  const tvItems = series.slice(0, 25).map((t: any) => {
    ...
    const castLinks   = castNames
      .map((name: string) =>
        `<a href="${escapeXml(`${currentSite}/tv/cast/${encodeURIComponent(name.trim())}`)}">${escapeXml(name)}</a>`
      )
      .join('، ');
    const directorHtml = t.data.director
      ? `<p><strong>إخراج:</strong> ${escapeXml(t.data.director)}</p>`
      : '';
    const castHtml = castLinks
      ? `<p><strong>طاقم العمل:</strong> ${castLinks}</p>`
      : '';
    const overview = escapeXml(t.data.overview || `شاهد حلقات مسلسل ${t.data.title} على معاك سيما`);
    ...
    return {
      ...
      link:        `/tv/${t.data.slug}`,
      content:     `<img src="${escapeXml(thumbUrl)}" alt="${escapeXml(t.data.title)}" /><br/>${directorHtml}${castHtml}<p>${overview}</p>`,
      customData:  `<media:content
        xmlns:media="http://search.yahoo.com/mrss/"
        url="${escapeXml(thumbUrl)}"
        medium="image"
