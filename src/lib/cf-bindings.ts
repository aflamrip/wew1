 ...
 *   CDN_VIDEO → video CDN bucket
 *   SESSION → KV namespace for cache
...
interface CFEnv {
  ...
  R2: R2Bucket;
  /** R2 bucket: video CDN (mp4 + vtt) — binding name: "CDN_VIDEO" */
  CDN_VIDEO: R2Bucket;
  /** KV namespace: ndjson page cache — binding name: "SESSION" */
  SESSION: KVNamespace;
...
// ---------------------------------------------------------------------------
const KV_TTL_INDEX  = 60 * 60 * 4;   // 4 h
const KV_TTL_DETAIL = 60 * 60 * 12;  // 12 h
const KV_TTL_SEASON = 60 * 30;        // 30 min
 ...
 * Checks KV first; on miss reads R2 and writes result to KV with the given TTL.
 * Falls back to HTTP fetch if both R2 and KV are unavailable.
 */
async function cachedText(
  ...
  // 1. KV cache hit — uses SESSION binding
  try {
  ...
  // 2. R2 read — uses R2 binding (maps to static.ma3ak.top)
